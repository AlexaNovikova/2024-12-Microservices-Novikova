...
public interface OrderMapper {
    ...
    default Order mapMock(AddItemRequestDto dto, ShopUser userCtx) {
        Order order = Order.builder()
        ...
        var value = productPriceMap.getOrDefault(dto.getProductId(), BigDecimal.valueOf(100));

        OrderItem item = OrderItem.builder()
                .id(new OrderItemId(null, dto.getProductId()))
                .name("Product " + dto.getProductId())
                .price(value)
                .quantity(1)
                .order(order) // Устанавливаем связь с Order
                .build();
